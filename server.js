...
app.post("/cart", auth, async (req, res) => {
    const { toyId, quantity, action } = req.body; // action: 'add' (default for new items) or 'set'
    const user = await User.findById(req.session.user.id);

    // toyId is Number in schema
...
app.post("/toys", adminAuth, async (req, res) => {
    // manual id or auto-increment? For simplicity, let's max(id)+1
...
// Seed Route (to load initial data if empty)
...
app.post("/checkout", auth, async (req, res) => {
    ...
    // Fetch toy details for snapshot
    // We need "id" (custom id) to find toy
    ...
    const orderItems = user.cart.map(cartItem => {
        ...
        // Decrease stock?
        // if(toy.stock >= cartItem.quantity) {
        //     toy.stock -= cartItem.quantity;
        //     toy.save(); 
        // }
